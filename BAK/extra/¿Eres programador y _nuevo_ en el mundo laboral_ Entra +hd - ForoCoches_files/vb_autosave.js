...
try
{
	...
	autosave.innerHTML = "Auto-Guardado: Activo";
	}
		...
		if (formObject.message.value == "" || formObject.id == 'message_form') {
	...
	// Info
	autosave.innerHTML = "Auto-Guardado: Mensaje recuperado [ <a href=# onclick='clearStorage(); return false'>Olvidar</a> ]";
		...
		function writeToEditor()
		{
			...
			//console.log('AutoSave :: Auto-fetched from storage ' + storageKey);
				...
				// Info
				if(document.getElementById('autosave')) {
				autosave.innerHTML = "Auto-Guardado: Mensaje recuperado";
				}
		...
		function clearStorage()
		{
			...
			autosave.innerHTML = "Auto-Guardado: Mensaje olvidado";
		}
		...
		function insertData()
		{
			...
			else if (formObject && formObject.message)
			{
				if (storage.getItem(storageKey) && storage.getItem(storageKey) != formObject.message.value)
				{
					...
					{
						if (confirm('Se ha encontrado un borrador de mensaje guardado. Quieres recuperar el contenido?\n\n' + storage.getItem(storageKey)))
						{
							writeToTextarea();
						}
						else
